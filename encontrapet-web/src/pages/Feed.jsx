...
export default function Feed() {
    ...
    const anunciosFiltrados = anuncios.filter((ad) => {
        ...
        const nomeOuTitulo = isPerdido ? ad.animal?.nome : ad.titulo;
        const especie = isPerdido ? ad.animal?.especie : 'Desconhecida';
        const cidadeEstado = `${ad.cidade} ${ad.estado}`.toLowerCase();
        const termoBusca = busca.toLowerCase();
        const matchBusca =
            nomeOuTitulo?.toLowerCase().includes(termoBusca) ||
            cidadeEstado.includes(termoBusca);

        let matchCategoria = true;
        if (filtroAtivo === 'Cachorros') matchCategoria = especie?.toLowerCase() === 'cachorro';
        if (filtroAtivo === 'Gatos') matchCategoria = especie?.toLowerCase() === 'gato';
        if (filtroAtivo === 'Apenas Perdidos') matchCategoria = isPerdido;
        if (filtroAtivo === 'Apenas Encontrados') matchCategoria = !isPerdido;
        ...
        <div className="min-h-screen bg-gray-100 font-sans pb-10">
            ...
            <header className="bg-indigo-500 text-white sticky top-0 z-50 shadow-md">
                <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-20 flex items-center justify-between">
                    ...
                    <div className="flex items-center space-x-4">
                        <Link to="/novo-anuncio" className="hidden sm:flex items-center bg-white text-indigo-500 px-4 py-2 rounded-full font-semibold hover:bg-indigo-50 transition">
            ...
            <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mt-8 pb-12">
                <div className="flex flex-col-reverse lg:flex-row gap-8">
                    ...
                    {/* COLUNA ESQUERDA: GRID DE CARDS */}
                    <div className="flex-1">
                        <div className="flex justify-between items-center mb-6 border-b pb-2">
                            <h2 className="text-2xl font-bold text-gray-800">Últimos Registros</h2>
                            <span className="text-sm font-medium text-gray-500 bg-gray-200 px-3 py-1 rounded-full">
                                {anunciosFiltrados.length} encontrados
                            </span>
                        </div>
                        ...
                        {loading ? (
                            ...
                            <div className="text-center mt-20 text-gray-500 bg-white p-10 rounded-3xl shadow-sm border border-gray-100">
                                ...
                                <p className="mt-2">Tente ajustar os filtros ou a sua busca.</p>
                            </div>
                        ) : (
                            <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-6">
                                {/* Agora usamos anunciosFiltrados no map! */}
                                    ...
                                    return (
                                        <div key={ad.id} className="bg-white rounded-2xl shadow-sm hover:shadow-xl transition-shadow duration-300 overflow-hidden border border-gray-100 flex flex-col">
                                            ...
                                            <div className="p-4 flex-1 flex flex-col">
                                                ...
                                                <Link
                                                    to={`/anuncio/${ad.id}`}
                                                    className="w-full mt-auto bg-green-500 hover:bg-green-600 text-white py-2 rounded-xl font-bold transition flex justify-center items-center text-sm cursor-pointer"
                                                >
                    ...
                    <div className="lg:w-1/3 xl:w-[400px]">
                        <div className="sticky top-24 bg-white p-4 rounded-3xl shadow-sm border border-gray-100">
                            ...
                            <div className="h-[450px] rounded-xl overflow-hidden relative z-0">
                                {/* Enviamos a lista filtrada para o mapa também! */}
                                <MapaFeed anuncios={anunciosFiltrados} />
                            </div>
